...
img = cv.imread(filename, cv.IMREAD_UNCHANGED)  # keep binary or grayscale as-is
...
#Added part
#Make sure that it is a binary image
_, binary = cv.threshold(img, 127, 255, cv.THRESH_BINARY)

#Find contours with hierarchy
contours, hierarchy = cv.findContours(binary, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE)

#Count tubules inside each cilie
tubules_per_cilie = {}
for i, h in enumerate(hierarchy[0]):
    parent = h[3]  #h[3] is the index of the parent contour
    if parent != -1:  #It’s a tubule (inside another contour)
        tubules_per_cilie[parent] = tubules_per_cilie.get(parent, 0) + 1

#Results
print("Tubules in each cilie:")
for cilie_id, count in tubules_per_cilie.items():
    print(f"  Cilie {cilie_id}: {count} tubule(s)")
